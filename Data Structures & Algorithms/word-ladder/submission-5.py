class Solution:
    def ladderLength(self, beginWord: str, endWord: str, wordList: List[str]) -> int:
        ...
        # return 0
        # time: O(m*m * n) space: O(m * n) m=len(word), n= len(set(wordlist))

        wordSet = set(wordList) # T:O(n),  S:(n*m)
        if endWord not in wordSet or beginWord == endWord:
            return 0

        visited = set() # T:O(n) S:(n*m)
        visited.add(beginWord) 
        q = deque([(beginWord, 1)]) #S:(n*m)

        while q:
            word, steps = q.popleft() #BFS processes at most n words, T:O(n) 
            if word == endWord:
                return steps
            word_list = list(word)
            for i in range(len(word)): # T:O(m) => m is len word
                original = word_list[i]

                for ch in "abcdefghijklmnopqrstuvwxyz": # T:O(26)
                    if ch != original:
                        word_list[i] = ch
                        new_word = "".join(word_list) # T:O(m) => m is len word
